...
final as (
    ...
    select
        ...
        coalesce(g.max_playtime, 1)                             as max_playtime,
        coalesce(g.min_age, 3)                                  as min_age,
        coalesce(g.complexity_weight, 1.0)                      as complexity_weight,
        g.bgg_rank,

        coalesce(
            round((g.min_playtime + g.max_playtime) / 2, 0),
            g.max_playtime
        )                                                       as avg_playtime,
        ...
        coalesce(d.name, 'Unknown')                             as designer_name,
